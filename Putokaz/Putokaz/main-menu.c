...
static int delete_attempt_count = 0; // staticka varijabla za brojanje pokusaja brisanja destinacija
...
int main_menu(char* dest_file) {
	...
	while (1) {
		...
		case PRINT_DEST:
			...
			printf("4. Ispis prosjecne cijene svih destinacija\n");
			...
			case 3:
				...
				break;
			case 4:
				average_price = calculate_average_cost(dest_field, dest_count);
				printf("\nProsjecna cijena svih destinacija je: %.2f EUR\n", average_price);
				break;
			default:
		...
		case DELETE_FILE:
			...
			dest_count = 0; // Resetiraj brojac
			free(dest_field); // Oslobodi memoriju za polje destinacija
			dest_field = NULL;
			break;
